...
UPDATE sessions s SET
    event_count = COALESCE(
        (SELECT COUNT(*)::INTEGER FROM session_events e WHERE e.session_id = s.id),
        0
    ),
...
-- the same transaction as the INSERT so a failed UPDATE rolls back the
-- event row, which means the counters can never silently drift from
-- the actual event population.
