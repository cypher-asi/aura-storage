...
async fn query_stats(
    ...
    // Build the query dynamically based on scope column and optional agent filter.
    // Safe: scope_column is only ever "project_id" or "org_id" from our match.
    let agent_filter = if agent_id.is_some() {
        format!("AND project_agent_id IN (SELECT id FROM project_agents WHERE agent_id = $2)")
    } else {
        String::new()
    };
    let task_agent_filter = if agent_id.is_some() {
        format!("AND project_agent_id IN (SELECT id FROM project_agents WHERE agent_id = $2)")
    } else {
        String::new()
    };
    ...
    let sql = format!(
        ...
        col = scope_column,
        af = agent_filter,
        taf = task_agent_filter,
    );

    let mut query = sqlx::query_as::<_, ExecutionStats>(&sql).bind(scope_id);
    if let Some(aid) = agent_id {
        query = query.bind(aid);
    }

    let stats = query.fetch_one(pool).await?;
    Ok(stats)
}
